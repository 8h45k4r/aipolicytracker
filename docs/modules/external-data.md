...
| Local snapshot | Every successful run merges its rows by id into `external/aiid_live_incidents.json` and `external/aiid_live_reports.json` on the `local` disk (persistent storage in production, see [guides-and-downloads.md](guides-and-downloads.md) "File persistence"). `external:import` reads them after the repository files, newer `modified_at` winning, so a rebuilt database gets every live-synced record back without calling the API. |
| Fallback | Pages read only from the database, never from AIID at request time. If the API refuses requests or is down, the command exits non-zero, records the error for the admin page and leaves every row untouched; the weekly Excel/backup path and the local snapshot continue to populate the tables. If the database were rebuilt, `external:import` restores the repository files plus the local snapshot. |
