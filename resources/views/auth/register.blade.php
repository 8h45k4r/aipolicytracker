    ...
    <main class="flex items-center justify-center px-4 py-12">
        <div class="w-full max-w-sm">
            ...
            <form method="post" action="{{ route('register') }}" class="mt-6 space-y-4">
                ...
                <div><label for="email" class="label">Email address</label><input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="email" class="input"><p class="mt-1 text-xs text-brand-muted">Work or personal, whichever you read. Temporary and disposable mailboxes are not accepted, because a deadline alert sent to one never arrives.</p>@error('email')<p class="mt-1 text-xs text-state-bad">{{ $message }}</p>@enderror</div>
