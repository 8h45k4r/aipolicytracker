@props(['source' => 'site', 'topic' => null, 'compact' => false])
<form method="post" action="{{ route('subscribe.store') }}" {{ $attributes->merge(['class' => 'rule-strong pt-3']) }} aria-label="Subscribe to the weekly AI policy digest">
    ...
    <p class="mt-1 text-sm text-brand-body">{{ $topic ? 'Get dated, source-linked changes for this jurisdiction every week.' : 'Dated, source-linked AI policy changes and upcoming application dates, once a week. No marketing.' }}</p>
    ...
    <div class="mt-3 flex gap-2 {{ $compact ? '' : 'max-w-md' }}">
    ...
    <p class="mt-2 meta">Double opt-in; one-click unsubscribe in every email.</p>
</form>
