...
<ol {{ $attributes->merge(['class' => 'chain']) }} aria-label="From law to evidence">
    @foreach([
        ['Jurisdictions', $stats['jurisdictions'] ?? 0, route('jurisdictions.index'), 'Track the rule'],
        ['Instruments', $stats['policies'] ?? 0, route('policies.index'), 'Track the requirement'],
        ['Obligations', $stats['obligations'] ?? 0, route('obligations.index'), 'Understand the duty'],
        ['Controls', $stats['controls'] ?? 0, route('controls.index'), 'Operationalise it'],
        ['Evidence types', $stats['evidence_types'] ?? 0, route('controls.index').'#evidence', 'Prove it'],
