...
<figure {{ $attributes->merge(['class' => 'card-flat p-4']) }}>
    ...
    <svg viewBox="0 0 {{ $w }} {{ $height + 70 }}" role="img" aria-label="{{ $title }}" class="mt-2 w-full max-w-full h-auto">
        ...
        @php($x = $loop->index * $bw)
        @php($h = $height * $v / $max)
        <a href="{{ route('risk.incidents.browse', ['year' => $year]) }}"><rect x="{{ $x + 3 }}" y="{{ 40 + $height - $h }}" width="{{ $bw - 6 }}" height="{{ $h }}" fill="#002147" opacity="0.9"><title>{{ $year }}: {{ number_format($v) }} incidents</title></rect></a>
        ...
        <circle cx="{{ $x }}" cy="{{ 14 + ($i % 3) * 9 }}" r="3" fill="#00A3E0"><title>{{ $m[0] }}: {{ $m[1] }}</title></circle>
        @endif
        @endforeach
    </svg>
