...
@if(!empty($submission->payload['field']))<p style="margin:0 0 8px;font-size:13px;"><strong>Field:</strong> {{ $submission->payload['field'] }}<br><strong>Shown:</strong> {{ \Illuminate\Support\Str::limit($submission->payload['current_value'] ?? '—', 400) }}<br><strong>Proposed:</strong> {{ \Illuminate\Support\Str::limit($submission->payload['proposed_value'] ?? '—', 400) }}</p>@endif
@if(!empty($submission->payload['record_url']))<p style="margin:0 0 8px;font-size:13px;">Record: <a href="{{ $submission->payload['record_url'] }}" style="color:#006AAC;">{{ $submission->payload['record_title'] ?? $submission->payload['record_url'] }}</a></p>@endif
