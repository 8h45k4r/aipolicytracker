        ...
        <ul class="mt-3 grid gap-4 md:grid-cols-2">
            ...
            <li class="card-flat p-5 flex flex-col">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="badge bg-brand-paper text-brand-body ring-brand-line">{{ config('frameworks.'.$f['key'].'.kind') ? ucfirst(str_replace('_', ' ', config('frameworks.'.$f['key'].'.kind'))) : 'Framework' }}</span>
                ...
                @if($f['obligations'] > 0)
                    <dl class="mt-4 grid grid-cols-4 gap-2 text-center">
                        ...
                        <div class="rounded-md bg-brand-paper py-2"><dt class="text-xs text-brand-muted">Controls</dt><dd class="text-xl font-semibold text-brand-navy">{{ $controlCounts[$f['key']] ?? 0 }}</dd></div>
                    </dl>
                ...
                @elseif(($controlCounts[$f['key']] ?? 0) > 0)
                    <p class="mt-4 text-sm text-brand-body">No legal duty is crosswalked to it directly; {{ $controlCounts[$f['key']] }} {{ \Illuminate\Support\Str::plural('control', $controlCounts[$f['key']]) }} cite it. <a href="{{ route('controls.index') }}">See the controls</a>.</p>
                @else
