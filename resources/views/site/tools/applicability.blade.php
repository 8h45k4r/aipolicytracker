    ...
    <div class="mt-6 grid gap-10 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 min-w-0">
            ...
            @else
                ...
                @php($resultControls = $result['obligations']->flatMap(fn ($o) => $o->controls->filter(fn ($c) => $c->published_at)->map(fn ($c) => ['control' => $c, 'satisfies' => $c->pivot->relationship === 'satisfies']))->groupBy(fn ($r) => $r['control']->slug)->map(fn ($rows) => ['control' => $rows->first()['control'], 'satisfies' => $rows->where('satisfies', true)->count(), 'duties' => $rows->count()])->sortByDesc(fn ($r) => [$r['satisfies'], $r['duties']])->values())
                @if($resultControls->isNotEmpty())
                <section aria-labelledby="r-controls" class="mt-8"><h2 id="r-controls" class="section-title">Controls to build first</h2>
                    <p class="mt-1 text-xs text-brand-muted">The controls that meet the obligations above, sorted by how many of them each one satisfies. Each control page lists the evidence a reviewer would expect.</p>
                    <ul class="mt-2 divide-y divide-brand-line border-y border-brand-line text-sm">@foreach($resultControls->take(8) as $r)<li class="py-2.5 flex items-baseline justify-between gap-3"><span><a href="{{ $r['control']->url() }}" class="font-medium text-brand-navy hover:underline">{{ $r['control']->title }}</a> <span class="text-xs text-brand-muted">{{ $r['control']->kindLabel() }} · {{ $r['control']->owner_role }}</span></span><span class="font-mono text-xs tabular-nums text-brand-muted whitespace-nowrap">{{ $r['satisfies'] }} / {{ $r['duties'] }} duties</span></li>@endforeach</ul>
                </section>
                @endif
