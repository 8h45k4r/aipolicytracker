...
<div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mt-6 grid gap-4 sm:grid-cols-2 text-sm">
        ...
        <div class="card-flat p-4"><p class="font-semibold text-brand-navy">Contributor expectations</p><p class="mt-1 text-brand-body">Official or openly licensed sources only, your own words, no legal conclusions, honest verification status, and no personal data. See <a href="{{ config('aipolicytracker.github_url') }}/blob/main/CONTRIBUTING.md" rel="noopener">CONTRIBUTING.md</a>, <a href="{{ config('aipolicytracker.github_url') }}/blob/main/SOURCE_ATTRIBUTION.md" rel="noopener">SOURCE_ATTRIBUTION.md</a>, the <a href="{{ config('aipolicytracker.github_url') }}/blob/main/GOVERNANCE.md" rel="noopener">governance policy</a> and the <a href="{{ config('aipolicytracker.github_url') }}/blob/main/CODE_OF_CONDUCT.md" rel="noopener">code of conduct</a>.</p></div>
    </div>
    ...
    <form method="post" action="{{ route('contribute.store') }}" class="mt-8 card-flat p-4 sm:p-6 space-y-4" data-track="submission_submit">
        ...
        @else
        <div class="grid gap-4 sm:grid-cols-2">
            <div><label for="s-subject-type" class="label">Related record type</label><select id="s-subject-type" name="subject_type" class="input"><option value="">Not specific</option>@foreach(['policy','jurisdiction','obligation','change','incident','risk','other'] as $t)<option value="{{ $t }}" @selected(old('subject_type', $prefill['subject_type']) === $t)>{{ ucfirst($t) }}</option>@endforeach</select></div>
            <div><label for="s-subject-slug" class="label">Record slug (from the URL)</label><input id="s-subject-slug" name="subject_slug" class="input" value="{{ old('subject_slug', $prefill['subject_slug']) }}" placeholder="e.g. eu-ai-act" pattern="[A-Za-z0-9._-]*"><p class="mt-1 text-xs text-brand-muted">Tip: use the "Report a correction" button on a record page and the form fills this in for you.</p></div>
        </div>
        @endif
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            ...
            <div><label for="s-email" class="label">Email <span class="text-brand-muted font-normal">(optional, for follow-up)</span></label><input id="s-email" name="submitter_email" type="email" class="input" value="{{ old('submitter_email') }}"><p class="mt-1 text-xs text-brand-muted">Leave it blank to submit anonymously. If you give one it has to be a mailbox that can reply, so a reviewer can come back with a question.</p>@error('submitter_email')<p class="mt-1 text-xs text-state-bad">{{ $message }}</p>@enderror</div>
