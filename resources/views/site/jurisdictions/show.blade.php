...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
    ...
    <header class="mt-3">
        ...
        <h1 class="mt-1 text-2xl sm:text-3xl lg:text-4xl font-semibold tracking-tight text-slate-900">AI regulation in {{ $jurisdiction->nameWithArticle() }}</h1>
        <div class="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm"><x-site.verified :record="$jurisdiction" class="!text-sm" /><span class="text-slate-600">{{ $policies->count() }} {{ \Illuminate\Support\Str::plural('instrument', $policies->count()) }} · {{ $policies->where('is_binding', true)->count() }} binding</span></div>
    </header>
    ...
    <div class="mt-8 grid gap-10 lg:grid-cols-3">
        <div class="lg:col-span-2 min-w-0">
            ...
            <section aria-labelledby="policies-heading" class="mt-8">
                ...
                @if($policies->count() > 3)<a href="{{ route('policies.index', ['jurisdiction' => $jurisdiction->slug]) }}" class="mt-3 inline-block text-sm text-teal-800 hover:underline">Filter all {{ $jurisdiction->name }} policies</a>@endif
            </section>
            ...
            <section aria-labelledby="changes-heading" class="mt-8">
                ...
                <div class="mt-1 divide-y divide-slate-200 border-y border-slate-200">@forelse($changes as $c)<x-site.change-item :change="$c" compact />@empty<p class="py-4 text-sm text-slate-600">No change events recorded yet.</p>@endforelse</div>
            </section>
            ...
            <section aria-labelledby="applies-heading" class="mt-8">
                ...
                <dl class="mt-3 grid gap-4 sm:grid-cols-3 text-sm">
                    <div><dt class="text-slate-500">Use cases</dt><dd class="mt-1 flex flex-wrap gap-1.5">@foreach($useCases as $t)<a class="chip !min-h-0 !py-1" href="{{ route('policies.index', ['jurisdiction' => $jurisdiction->slug, 'use_case' => $t->slug]) }}">{{ $t->name }}</a>@endforeach</dd></div>
                    <div><dt class="text-slate-500">Sectors</dt><dd class="mt-1 flex flex-wrap gap-1.5">@foreach($sectors as $t)<a class="chip !min-h-0 !py-1" href="{{ route('policies.index', ['jurisdiction' => $jurisdiction->slug, 'sector' => $t->slug]) }}">{{ $t->name }}</a>@endforeach</dd></div>
                    <div><dt class="text-slate-500">Obligation areas</dt><dd class="mt-1 flex flex-wrap gap-1.5">@foreach($obligationCategories as $c)<a class="chip !min-h-0 !py-1" href="{{ route('obligations.index', ['jurisdiction' => $jurisdiction->slug, 'category' => $c->category]) }}">{{ str_replace('_', ' ', $c->category) }} ({{ $c->n }})</a>@endforeach</dd></div>
                </dl>
            </section>
            @endif
            ...
            <section aria-labelledby="howto-heading" class="mt-8">
                ...
                <ol class="mt-2 list-decimal space-y-1.5 pl-5 text-sm text-slate-700">@foreach($jurisdiction->how_to_use as $step)<li>{{ $step }}</li>@endforeach</ol>
            </section>
            @endif
        ...
        <aside class="space-y-6">
            <div class="lg:sticky lg:top-4 space-y-6">
                ...
                <div class="card-flat p-4 text-sm"><p class="font-semibold text-slate-900">Regulators</p><ul class="mt-2 space-y-2">@foreach($jurisdiction->regulators as $r)<li><a href="{{ $r['url'] }}" rel="noopener" class="text-slate-900 hover:underline" data-track="source_click">{{ $r['name'] }}</a>@if(!empty($r['role']))<div class="text-xs text-slate-500">{{ $r['role'] }}</div>@endif</li>@endforeach</ul></div>
                @endif
                @if($related->isNotEmpty())
                <div class="text-sm"><p class="font-semibold text-slate-900">Compare with</p><ul class="mt-2 space-y-1.5">@foreach($related as $r)<li><a href="{{ route('compare.index', ['j' => $jurisdiction->slug.','.$r->slug]) }}" class="text-slate-800 hover:underline" rel="nofollow">{{ $jurisdiction->short_name ?: $jurisdiction->name }} vs {{ $r->short_name ?: $r->name }}</a></li>@endforeach</ul></div>
                @endif
                @if($jurisdiction->children->isNotEmpty())
                <div class="text-sm"><p class="font-semibold text-slate-900">Sub-national jurisdictions</p><ul class="mt-2 space-y-1.5">@foreach($jurisdiction->children as $c)<li><a href="{{ $c->url() }}" class="text-slate-800 hover:underline">{{ $c->name }}</a></li>@endforeach</ul></div>
                @endif
