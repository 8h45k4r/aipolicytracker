...
<div class="container-site py-8">
    ...
    <header class="mt-3">
        <div class="flex flex-wrap gap-1.5"><span class="badge bg-brand-navy text-white ring-brand-navy">{{ $types[$tool['type']] }}</span><span class="badge bg-state-goodbg text-state-good ring-state-good/30">Free download</span>@foreach($tool['frameworks'] as $f)<span class="badge-neutral">{{ $fw[$f] ?? $f }}</span>@endforeach</div>
        ...
        <div class="mt-4 flex flex-wrap gap-2"><a href="#preview" class="btn-secondary">Preview {{ strtolower($types[$tool['type']]) }}</a>
            @auth<a href="#download" class="btn-primary">Download free {{ strtolower($types[$tool['type']]) }}</a>@else<a href="{{ route('tools.gate', $tool['slug']) }}" class="btn-primary" data-track="download_click" data-track-label="{{ $tool['slug'] }}">Download free {{ strtolower($types[$tool['type']]) }}</a>@endauth</div>
        <p class="mt-2 meta">Formats: {{ implode(', ', array_map(fn ($f) => $f[1], $tool['files'])) }} · Version {{ $tool['version'] }} · Last updated {{ \Carbon\Carbon::parse($tool['updated'])->format('j M Y') }}</p>
    </header>
    <div class="mt-8 grid gap-10 lg:grid-cols-3">
        <div class="lg:col-span-2 min-w-0">
            ...
            <section id="preview" class="mt-8" aria-labelledby="preview-heading">
                <h2 id="preview-heading" class="section-title">Preview: fields in the {{ strtolower($types[$tool['type']]) }}</h2>
            ...
            <section id="download" class="mt-10 card-flat p-5" aria-labelledby="dl-heading">
                @auth
                <h2 id="dl-heading" class="section-title">Download free {{ strtolower($types[$tool['type']]) }}</h2>
                <form method="post" action="{{ route('tools.download', $tool['slug']) }}" class="mt-3 space-y-3 text-sm">@csrf
                    ...
                    <button type="submit" class="btn-primary" data-track="download_submit" data-track-label="{{ $tool['slug'] }}">Get the files ({{ implode(', ', array_map(fn ($f) => $f[1], $tool['files'])) }})</button>
                ...
                <p class="mt-2 text-sm text-brand-body">Get instant access to this free {{ strtolower($types[$tool['type']]) }} and receive updates when related AI policy requirements change.</p>
            ...
            <div class="lg:sticky lg:top-4 space-y-6">
                <div class="card-flat p-4 text-sm"><p class="font-semibold text-brand-navy">At a glance</p>
                    <dl class="mt-2 space-y-1.5 text-brand-body">
                        <div class="flex justify-between gap-2"><dt class="text-brand-muted">Type</dt><dd>{{ $types[$tool['type']] }}</dd></div>
                        <div class="flex justify-between gap-2"><dt class="text-brand-muted">Formats</dt><dd class="text-right">{{ implode(', ', array_map(fn ($f) => $f[1], $tool['files'])) }}</dd></div>
                        <div class="flex justify-between gap-2"><dt class="text-brand-muted">Version</dt><dd>{{ $tool['version'] }}</dd></div>
                        <div class="flex justify-between gap-2"><dt class="text-brand-muted">Updated</dt><dd>{{ \Carbon\Carbon::parse($tool['updated'])->format('j M Y') }}</dd></div>
                ...
                <div class="flex flex-col gap-2 text-sm"><x-site.save-button type="tool" :slug="$tool['slug']" :title="$tool['title']" :url="route('tools.show', $tool['slug'])" meta="Free tool" /><button type="button" class="btn-secondary" data-copy-link>Copy link</button></div>
                @if($next)<div class="text-sm"><p class="font-semibold text-brand-navy">Next step</p><p class="mt-1"><a href="{{ route('tools.show', $next['slug']) }}" class="text-brand-navy">Download the {{ $next['title'] }} →</a></p></div>@endif
