...
<div class="container-site py-12 max-w-2xl">
    ...
    <p class="mt-3 meta">Links are personal and expire in 30 minutes; come back to the <a href="{{ route('tools.show', $tool['slug']) }}">tool page</a> to get fresh ones. Every file states its version, date and that it is informational only.</p>
    @if($next)<div class="mt-8 card-flat p-4 text-sm"><p class="font-semibold text-brand-navy">Next step</p><p class="mt-1"><a href="{{ route('tools.show', $next['slug']) }}" class="text-brand-navy">Download the {{ $next['title'] }} →</a></p></div>@endif
