...
<div class="container-site py-12 max-w-xl">
    ...
    <p class="mt-3 text-brand-body leading-7">Get instant access to the free <strong>{{ $tool['title'] }}</strong> ({{ implode(', ', array_map(fn ($f) => $f[1], $tool['files'])) }}) and receive updates when related AI policy requirements change.</p>
    ...
    <p class="mt-6 text-sm"><a href="{{ route('tools.show', $tool['slug']) }}">Back to the preview</a></p>
</div>
@endsection
